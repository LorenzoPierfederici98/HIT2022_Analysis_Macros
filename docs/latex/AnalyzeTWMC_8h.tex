...
Macro that fits the charge distribution from the Analyze\+TWCharge\+Time.\+cc merged output files (for MC runs). A fit is performed with 2 separate gaussians, one for proton and one for helium peaks. The peaks are automatically found with TSpectrum in a certain range (between 1. and 12., the bins outside the range are set to 0); the peaks are then fitted within a certain bin-\/range centered around the peak. The fit results are stored in files name like e.\+g. {\ttfamily TW/\+Ana\+FOOT\+\_\+\+TW\+\_\+\+Decoded\+MC\+\_\+\+HIT2022\+\_\+\+MC\+\_\+140\+\_\+\+Fit.\+root}. To be run with `root -\/l -\/b -\/q \textquotesingle{}Analyze\+TWMC.\+cc()'\`{}. 
