...
Include dependency graph for Calibrate\+Fragm.\+h\+:\nopagebreak
\begin{figure}[H]
\begin{center}
\leavevmode
\includegraphics[width=350pt]{CalibrateFragm_8h__incl}
\end{center}
\end{figure}
...
Macro that plots the fit results on protons and helium peaks given by the Analyze\+TWFragm.\+cc macro vs the energies retrieved from MC (2 energy loss values per bar and beam energy). The fit charge values vs energy loss values are fitted with a 1 parameter linear function; these parameters (one per bar) are written in the configuration file. To be run with `root -\/l -\/b -\/q \textquotesingle{}Calibrate\+Fragm.\+cc()'\`{}. 
